...
ha.prototype.noteOn=function(){var c=this.ctx,a=this.instrument,b=this.buffer,d,e,g,f,k,s=this.ctx.currentTime,p=s+a.volAttack,x=s+a.modAttack,A=p+a.volDecay,H=a.loopStart/this.sampleRate,O=a.loopEnd/this.sampleRate,I=a.start/this.sampleRate,b=b.subarray(0,b.length+a.end);d=this.audioBuffer=c.createBuffer(1,b.length,this.sampleRate);d.getChannelData(0).set(b);e=this.bufferSource=c.createBufferSource();e.buffer=d;e.loop=9!==this.channel;e.loopStart=H;e.loopEnd=O;ia(this,this.pitchBend,aa);g=this.panner=
...
(this.pitchBend/(0>this.pitchBend?8192:8191))+this.modEnvToPitch)*this.instrument.scaleTuning),x);e.connect(g);g.connect(f);f.connect(this.destination);e.start(0,I)};
ha.prototype.noteOff=function(){var c=this.instrument,a=this.bufferSource,b=this.gainOutput,d=this.ctx.currentTime,e=d+c.volRelease;this.audioBuffer&&(b.gain.cancelScheduledValues(0),b.gain.setValueAtTime(b.gain.value,d),b.gain.linearRampToValueAtTime(0,e),a.stop(e),setTimeout(function(a){return function(){a.bufferSource.disconnect(0);a.panner.disconnect(0);a.gainOutput.disconnect(0)}}(this),1E3*c.volRelease))};
...
x=s.offset,A;"LIST"!==s.type&&h(Error("invalid chunk type:"+s.type));A=String.fromCharCode(p[x++],p[x++],p[x++],p[x++]);"INFO"!==A&&h(Error("invalid signature:"+A));(new Ca(p,{index:x,length:s.size-4})).parse();var H=E(e,1),O,I=this.input,P=H.offset,ja;"LIST"!==H.type&&h(Error("invalid chunk type:"+H.type));ja=String.fromCharCode(I[P++],I[P++],I[P++],I[P++]);"sdta"!==ja&&h(Error("invalid signature:"+ja));O=new Ca(I,{index:P,length:H.size-4});O.parse();1!==O.chunkList.length&&h(Error("TODO"));this.samplingData=
...
2);"pmod"!==ca.type&&h(Error("invalid chunk type:"+ca.type));this.presetZoneModulator=Fa(this,ca);var G=E(v,3);"pgen"!==G.type&&h(Error("invalid chunk type:"+G.type));this.presetZoneGenerator=Ia(this,G);var Z=E(v,4),C=this.input,Q=Z.offset,za=this.instrument=[],Aa=Z.offset+Z.size;"inst"!==Z.type&&h(Error("invalid chunk type:"+Z.type));for(;Q<Aa;)za.push({instrumentName:String.fromCharCode.apply(ba,C.subarray(Q,Q+=20)),instrumentBagIndex:C[Q++]|C[Q++]<<8});var J=E(v,5),R=this.input,z=J.offset,la=this.instrumentZone=
[],ma=J.offset+J.size;"ibag"!==J.type&&h(Error("invalid chunk type:"+J.type));for(;z<ma;)la.push({instrumentGeneratorIndex:R[z++]|R[z++]<<8,instrumentModulatorIndex:R[z++]|R[z++]<<8});var $=E(v,6);"imod"!==$.type&&h(Error("invalid chunk type:"+$.type));this.instrumentZoneModulator=Fa(this,$);var na=E(v,7);"igen"!==na.type&&h(Error("invalid chunk type:"+na.type));this.instrumentZoneGenerator=Ia(this,na);var K=E(v,8),m=this.input,l=K.offset,Ba=this.sample=[],n=this.sampleHeader=[],oa=K.offset+K.size,
S,da,pa,T,U,V,qa,ra,sa,ta;"shdr"!==K.type&&h(Error("invalid chunk type:"+K.type));for(;l<oa;){S=String.fromCharCode.apply(ba,m.subarray(l,l+=20));da=(m[l++]<<0|m[l++]<<8|m[l++]<<16|m[l++]<<24)>>>0;pa=(m[l++]<<0|m[l++]<<8|m[l++]<<16|m[l++]<<24)>>>0;T=(m[l++]<<0|m[l++]<<8|m[l++]<<16|m[l++]<<24)>>>0;U=(m[l++]<<0|m[l++]<<8|m[l++]<<16|m[l++]<<24)>>>0;V=(m[l++]<<0|m[l++]<<8|m[l++]<<16|m[l++]<<24)>>>0;qa=m[l++];ra=m[l++]<<24>>24;sa=m[l++]|m[l++]<<8;ta=m[l++]|m[l++]<<8;var ea=new Int16Array((new Uint8Array(m.subarray(this.samplingData.offset+
2*da,this.samplingData.offset+2*pa))).buffer);T-=da;U-=da;if(0<V){for(var L=ea,M=V,fa=r,ua=r,Na=r,Ga=r,Ha=1;22050>M;){fa=new Int16Array(2*L.length);ua=Ga=0;for(Na=L.length;ua<Na;++ua)fa[Ga++]=L[ua],fa[Ga++]=L[ua];L=fa;Ha*=2;M*=2}c=L;a=Ha;ea=c;V*=a;T*=a;U*=a}Ba.push(ea);n.push({sampleName:S,startLoop:T,endLoop:U,sampleRate:V,originalPitch:qa,pitchCorrection:ra,sampleLink:sa,sampleType:ta})}this.input=ba};
...
Ua.prototype.onload=function(c){var a,b,d,e,g=new Uint8Array(c),f=this.synth=new La(g),k=Pa(f),s;f.parser=new Ea(f.input);var p=f.parser;p.parse();var x=p.presetHeader,A=p.presetZone,H=[],O,I,P,ja,B,v,w,W;B=0;for(v=x.length;B<v;++B){O=x[B].presetBagIndex;I=x[B+1]?x[B+1].presetBagIndex:A.length;P=[];w=O;for(W=I;w<W;++w){var ka=Ka(A[w].presetGeneratorIndex,A[w+1]?A[w+1].presetGeneratorIndex:p.presetZoneGenerator.length,p.presetZoneGenerator);d=ka.modgen;e=ka.modgenInfo;var X=Ka(A[w].presetModulatorIndex,
A[w+1]?A[w+1].presetModulatorIndex:p.presetZoneModulator.length,p.presetZoneModulator);a=X.modgen;b=X.modgenInfo;P.push({generator:d,generatorSequence:e,modulator:a,modulatorSequence:b});ja=d.instrument!==r?d.instrument.amount:a.instrument!==r?a.instrument.amount:ba}H.push({name:x[B].presetName,info:P,header:x[B],instrument:ja})}var u,q,va,wa,F=p.instrument,D=p.instrumentZone,Y=[],xa,ya,ca,G,Z,C,Q;G=0;for(Z=F.length;G<Z;++G){xa=F[G].instrumentBagIndex;ya=F[G+1]?F[G+1].instrumentBagIndex:D.length;
ca=[];C=xa;for(Q=ya;C<Q;++C){var za=Ka(D[C].instrumentGeneratorIndex,D[C+1]?D[C+1].instrumentGeneratorIndex:p.instrumentZoneGenerator.length,p.instrumentZoneGenerator);va=za.modgen;wa=za.modgenInfo;var Aa=Ka(D[C].presetModulatorIndex,D[C+1]?D[C+1].instrumentModulatorIndex:p.instrumentZoneModulator.length,p.instrumentZoneModulator);u=Aa.modgen;q=Aa.modgenInfo;ca.push({generator:va,generatorSequence:wa,modulator:u,modulatorSequence:q})}Y.push({name:F[G].instrumentName,info:ca})}var J=[],R,z,la,ma,$,
na,K,m;$=0;for(na=H.length;$<na;++$)if(z=H[$],ma=z.header.preset,"number"===typeof z.instrument&&(la=Y[z.instrument],"EOI"!==la.name.replace(/\0*$/,""))){J[z.header.bank]===r&&(J[z.header.bank]=[]);R=J[z.header.bank];R[ma]=[];R[ma].name=z.name;K=0;for(m=la.info.length;K<m;++K){var l=p,Ba=R[ma],n=la.info[K].generator,oa=r,S=r,da=r,pa=r,T=r,U=r,V=r,qa=r,ra=r,sa=r,ta=r,ea=r,L=r,M=r,fa=r;if(!(n.keyRange===r||n.sampleID===r)){da=N(n,"attackVolEnv",-12E3);pa=N(n,"decayVolEnv",-12E3);T=N(n,"sustainVolEnv");
U=N(n,"releaseVolEnv",-12E3);V=N(n,"attackModEnv",-12E3);qa=N(n,"decayModEnv",-12E3);ra=N(n,"sustainModEnv");sa=N(n,"releaseModEnv",-12E3);ta=N(n,"coarseTune")+N(n,"fineTune")/100;ea=N(n,"scaleTuning",100)/100;L=N(n,"freqVibLFO");M=n.keyRange.lo;for(fa=n.keyRange.hi;M<=fa;++M)Ba[M]||(oa=N(n,"sampleID"),S=l.sampleHeader[oa],Ba[M]={sample:l.sample[oa],sampleRate:S.sampleRate,basePlaybackRate:Math.pow(Math.pow(2,1/12),(M-N(n,"overridingRootKey",S.originalPitch)+ta+S.pitchCorrection/100)*ea),modEnvToPitch:N(n,
"modEnvToPitch")/100,scaleTuning:ea,start:32768*N(n,"startAddrsCoarseOffset")+N(n,"startAddrsOffset"),end:32768*N(n,"endAddrsCoarseOffset")+N(n,"endAddrsOffset"),loopStart:S.startLoop+32768*N(n,"startloopAddrsCoarseOffset")+N(n,"startloopAddrsOffset"),loopEnd:S.endLoop+32768*N(n,"endloopAddrsCoarseOffset")+N(n,"endloopAddrsOffset"),volAttack:Math.pow(2,da/1200),volDecay:Math.pow(2,pa/1200),volSustain:T/1E3,volRelease:Math.pow(2,U/1200),modAttack:Math.pow(2,V/1200),modDecay:Math.pow(2,qa/1200),modSustain:ra/
1E3,modRelease:Math.pow(2,sa/1200),freqVibLFO:L?8.176*Math.pow(2,L/1200):r})}}}f.bankSet=J;for(s=0;16>s;++s)Ra(f,s,s),Sa(f,s,100),Ta(f,s,64),f.pitchBend(s,0,64),f.pitchBendSensitivity(s,2);f.start();document.body.appendChild(k);window.addEventListener("message",this.onmessage.bind(this),t);window.opener?window.opener.postMessage("link,ready","*"):window.parent!==window&&window.parent.postMessage("link,ready","*")};
